        ...
        init(from movement: Movement) {
            ...
            if let log = movement.latest_log, log.for_current_workout == true {
                ...
                self.logSets = templateSets.map { templateSet in
                    ...
                    let setType = EditableSet.SetType(rawValue: templateSet.type) ?? .working
                    return LogSet(reps: 0, load: nil, type: templateSet.type, rest_time: templateSet.rest_time ?? EditableSet.defaultRestTime(for: setType))
                }
                self.logNotes      = ""
                ...
                self.logSets       = previousSets.map { set in
                    let setType = EditableSet.SetType(rawValue: set.type) ?? .working
                    return LogSet(reps: 0, load: nil, type: set.type, rest_time: set.rest_time ?? EditableSet.defaultRestTime(for: setType))
                }
                self.logNotes      = ""
                self.existingLogId = nil
            } else {
                // No previous log, no template: seed one empty working set
                self.logSets       = [LogSet(reps: 0, load: nil, type: "working", rest_time: EditableSet.defaultRestTime(for: .working))]
                self.logNotes      = ""
                self.existingLogId = nil
            }
        }
